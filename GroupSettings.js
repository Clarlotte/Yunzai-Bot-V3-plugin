...
export class giveTitle extends plugin {
    ...
    async giveTitle(e) {
        let flag = 0;
        let str = e.raw_message;
        //由于本人不会js的读写文件，所以此处利用正则表达式判断违禁词，如果有大佬会，可以指导一下
        let RegArr = [/爸爸/g, /爹/g, /爷爷/g];
        var length = RegArr.length;
        let qq = Number(e.user_id);
        let title = str.slice(5, str.length);
        if (regtext.test(str)) {
            if (title.length > 6 || title.length <= 0) {
                ...
                for (var i = 0; i < length; i++) {
                    if (RegArr[i].test(title)) {
                        e.group.recallMsg(e);
                        e.group.muteMember(qq, 1800);
                        flag = 1;
                        let remsg = [
                            segment.at(qq),
                            `你申请的头衔中包含违禁词，你将被禁言30分钟`,
                        ];
                        e.reply(remsg)
                        return true;
                    }
